...
var individualRexp = new RegExp(
  "^(" + getKeys(individuals).sort(lenComp).join("|") + ")(\\d)?",
  "i"
);
  ...
  var parseToken = function (i) {
    ...
    if (parse) {
      ...
      if (typeof parse[3] !== "undefined") {
        ...
        detectedToneIdx = findBetween(
          sortedAccentedIndicies,
          i,
          i + parse[0].length
        );
...
const zhuyinInitials = new Set(
  "ㄅㄆㄇㄈㄉㄊㄋㄌㄍㄎㄏㄐㄑㄒㄓㄔㄕㄖㄗㄘㄙ".split("")
);
...
function getDifference(pinyin1, pinyin2) {
  const pair = pinyin1 > pinyin2 ? pinyin1 + pinyin2 : pinyin2 + pinyin1;
  if (pair in cache) {
    return cache[pair];
  }
  let zhuyin1 = pinyinToZhuyin(pinyin1);
  let zhuyin2 = pinyinToZhuyin(pinyin2);
  const difference = getZhuyinDifference(zhuyin1, zhuyin2);
  cache[pair] = difference;
  return difference;
}
