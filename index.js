...
function align(a, b, alignment, aOffset, bOffset, path) {
  ...
  align(
    ...
    rightB,
    alignment,
    aOffset + aIndex + commonLength,
    bOffset + bIndex + commonLength,
    path + "r"
  );
...
function getIntervals(transcriptWords, recognitionOutput) {
  let transcriptChars = transcriptWords
    .join("")
    .split("")
    .filter(isCharacterText)
    .join("");
  ...
  let recognitionChars = recogntionItems
    .map((x) => x.word)
    .join("")
    .split("")
    .filter(isCharacterText)
  ...
  fillMissingTimes(intervals);
  return intervals;
}

function fillMissingTimes(intervals) {
  let lastEnd = 0;
  for (let i = 0; i < intervals.length; i++) {
    let interval = intervals[i];
    if (interval.xmin !== null) {
      lastEnd = interval.xmax;
      continue;
    }
    let j = i + 1;
    while (j < intervals.length && intervals[j].xmin === null) {
      j++;
    }
    if (!intervals[j]) continue;
    nextStart = intervals[j].xmin;
    interval.xmin = lastEnd;
    interval.xmax = +(lastEnd + (nextStart - lastEnd) / (j - i)).toFixed(3);
    lastEnd = interval.xmax;
  }
}
